...
var KnownTokenInfo = {};
const KnownTokenList = require("../public/" +
  (config.settings.tokenList || "tokens.json"));
...
exports.data = async (req, res) => {
  try {
    if ("tx" in req.body) {
      ...
      if (transfer) {
        let contractDetail = await Contract.findOne(
          {
            $or: [
      ...
      transactionResponse.confirmations =
        latestBlock[0].number + 1 - transactionResponse.blockNumber;

      if (!transactionResponse.status) {
        transactionResponse.confirmations = 0;
      }
      transactionResponse.gasPriceGwei = web3.utils.fromWei(
        transactionResponse.gasPrice,
        "Gwei"
      );
      transactionResponse.gasPrice = web3.utils.fromWei(
        transactionResponse.gasPrice,
        "ether"
      );
      transactionResponse.transactionFee = new Number(
        transactionResponse.gasPrice * transactionResponse.gasUsed
      ).toFixed(20);
      transactionResponse.transactionFeeUSD = new Number(
        transactionResponse.transactionFee * quoteUSD
      ).toFixed(20);
      transactionResponse.transactionFeeINR = new Number(
        transactionResponse.transactionFee * quoteINR
      ).toFixed(20);
      transactionResponse.transactionFeequoteEUR = new Number(
        transactionResponse.transactionFee * quoteEUR
      ).toFixed(20);
      ...
      transactionResponse.gasUsedPercent =
        (transactionResponse.gasUsed / transactionResponse.gas) * 100;
      transactionResponse.tokensTrasferred = tokensTrasferred;

      if (
        transactionResponse.to === treasuryAddress ||
        transactionResponse.from === treasuryAddress
      ) {
        transactionResponse.inputAscii = web3.utils.hexToAscii(
        ...
        [
          function (callback) {
            web3.eth.getTransaction(txHash, function (err, tx) {
              if (err || !tx) {
                return callback(
                  { error: true, message: "Transaction not found." },
                  null
                );
              } else {
          ...
          function (tx, callback) {
            web3.trace.transaction(txHash, function (err, traces) {
              if (err || !traces) {
                ...
                return callback(
                  { error: true, message: "TraceWeb3 error:" + err },
                  null
                );
              } else {
          ...
          function (tx, traces, callback) {
            if (tx.to != null) {
              ...
              Contract.findOne({ address: tx.to })
                .lean(true)
          ...
          if (contractOrDb || tokenContract) {
            ...
            if (!tokenContract && typeof contractOrDb.abi == "string") {
              ...
              try {
                ...
                res.write(
                  JSON.stringify({ error: true, message: "normal TX" })
                );
                res.end();
                return;
              }
      ...
      asyncL.waterfall(
        [
          function (callback) {
            ...
            Transaction.findOne({ creates: addr })
              .lean(true)
              ...
              if (!transaction) {
                ...
                txns.forEach(function (t) {
                  if (t.type == "call") {
                    ...
                    var methodSig = t.action.input
                      ? t.action.input.substr(0, 10)
                      : null;
                    var callInfo = {};
                    if (
                      methodSig &&
                      KnownMethodIDs[methodSig] &&
                      KnownMethodIDs[methodSig].method == "transfer"
                    ) {
                    ...
                    var toAddr =
                      callInfo && callInfo.params && callInfo.params[0].value;
                ...
                txns.forEach(function (t) {
                  if (t.type == "call") {
                    ...
                    if (
                      ...
                      var amount = new BigNumber(callInfo.params[1].value);
                      t.amount = amount
                        .dividedBy(KnownTokenDecimalDivisors[addr])
                        .toString(10);
                      // replace to address with _to address arg
                      t.to = callInfo.params[0].value;
                      t.callInfo = callInfo;
    ...
    */
      ...
      if (req.body.action == "hashrate") {
        ...
        const activeAddressesStat = await ActiveAddressesStat.find()
          .sort({ blockNumber: -1 })
          .limit(1);
        const latestPrice = await Market.findOne().sort({ timestamp: -1 });
        let quoteUSD = 0;
