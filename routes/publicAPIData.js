...
var eth_getBlockTransactionCountByNumber =
  "eth_getBlockTransactionCountByNumber";
var eth_getTransactionByHash = "eth_getTransactionByHash";
var eth_getTransactionByBlockNumberAndIndex =
...
module.exports = async function (req, res) {
  ...
  try {
    ...
    if (!methodName) methodName = req.query.action;
    ...
    switch (methodName) {
      ...
      case xdc_marketprice:
        ...
        marketprice.exec(function (err, docs) {
          if (err) responseFail(res, respData, err.toString());
          else {
            sendData(res, respData, docs, format);
          }
        });
        break;
      ...
      case tokeninfo:
        ...
        Contract.findOne({ address: contractaddress }, [
          "address",
          "ERC",
          "blockNumber",
          "creationTransaction",
          ...
          if (err) {
            ...
            sendData(res, respData, doc, format);
      ...
      case tokentransfers:
        ...
        transfers = TokenTransfer.find({
          contract: {
            $regex: tokencontract.toString().toLowerCase(),
            $options: "i",
          },
        }).limit(limit);
      ...
      case totalXDC:
        ...
        if (onlyValue) {
          ...
          sendData(res, respData, value, format);
        }
        break;
      ...
      case balance:
        ...
        sendData(res, respData, await eth.getBalance(address), format);
        break;
      case balancemulti:
        addresses = requestParam(req, "address");
        addresses = addresses.split(",");
        if (addresses.length > 20) addresses.length = 20;
        var arr = [];
        ...
        sendData(res, respData, arr, format);
        break;
      ...
      case getminedblocks:
        ...
        var blockFind = Block.findOne({ miner: address })
          .skip(page * pageSize)
          .limit(pageSize)
          .lean(true);
      ...
      case getabi:
        ...
        Contract.findOne({ address: address }, "abi").exec(function (err, doc) {
          if (err) {
            ...
            sendData(res, respData, doc.abi, format);
          }
        });
        break;
      case getsourcecode:
        address = requestParam(req, "address");
        Contract.findOne({ address: address }, "sourceCode").exec(function (
          err,
          doc
        ) {
          if (err) {
            ...
            sendData(res, respData, doc.sourceCode, format);
          }
        });
        break;
      case getstatus:
        ...
        if (txr) {
          ...
          else data = { isError: "0", errDescription: "success" };
          sendData(res, respData, data, format);
        } else {
          responseFail(res, respData, "not exist");
        }
        break;
        ...
        if (isNaN(blockno)) {
          ...
          if (blockData) {
            var resultObj = {
              ...
              blockMiner: blockData.miner,
            };
            sendData(res, respData, resultObj, format);
          } else {
            respData.result = 0;
            responseFail(res, respData, "not exist");
          }
      ...
      case getLogs:
        ...
        if (toBlock) {
          if (isNaN(toBlock)) findObj.blockNumber = { $gte: fromBlock };
          else findObj.blockNumber = { $gte: fromBlock, $lte: toBlock };
        }
        if (topics) {
          var topicsArr = topics.split(",");
          for (var n = 0; n < topicsArr.length; n++) {
            topicsArr[n] = topicsArr[n].trim();
            if (topicsArr[n] != "") findObj["topics." + n] = topicsArr[n];
          }
        }
        if (data) {
          findObj.data = data;
        }
        if (toBlock == "latest") toBlock = totalBlockNum;
        ...
        } else findOP = LogEvent.find(findObj);
      ...
      case eth_blockNumber:
        sendData(res, respData, totalBlockNum, format);
        break;
      case eth_getBlockByNumber:
        blockNumber = requestParam(req, "blockNumber");
        if (blockNumber == "latest") blockNumber = totalBlockNum;
        else blockNumber = Number(blockNumber);
        sendData(res, respData, await eth.getBlock(blockNumber), format);
        break;
      case eth_getBlockTransactionCountByNumber:
        ...
        sendData(
          res,
          respData,
          await eth.getBlockTransactionCount(blockNumber),
          format
        );
        break;
      case eth_getTransactionByHash:
        txhash = requestParam(req, "txhash");
        sendData(res, respData, await eth.getTransaction(txhash), format);
        break;
      case eth_getTransactionByBlockNumberAndIndex:
        ...
        sendData(
          res,
          respData,
          await eth.getTransactionFromBlock(blockNumber, index),
          format
        );
        break;
      case eth_getTransactionCount:
        address = requestParam(req, "address");
        sendData(res, respData, await eth.getTransactionCount(address), format);
        break;
      case eth_sendRawTransaction:
        hex = requestParam(req, "hex");
        sendData(res, respData, await eth.sendSignedTransaction(hex), format);
        break;
      case eth_getTransactionReceipt:
        txhash = requestParam(req, "txhash");
        sendData(res, respData, await eth.getTransactionReceipt(txhash), format);
        break;
      case eth_call:
        to = requestParam(req, "to");
        data = requestParam(req, "data");
        blockNumber = requestParam(req, "blockNumber");
        sendData(res, respData, await eth.call({ to: to, data: data }), format);
        break;
      case eth_getCode:
        address = requestParam(req, "address");
        blockNumber = requestParam(req, "blockNumber");
        sendData(res, respData, await eth.getCode(address, blockNumber), format);
        break;
      case eth_getStorageAt:
        ...
        sendData(
          res,
          respData,
          await eth.getStorageAt(address, position, blockNumber),
          format
        );
        break;
      case eth_gasPrice:
        sendData(res, respData, await eth.gasPrice, format);
        break;
      case eth_estimateGas:
        var obj = {};
        from = requestParam(req, "from");
        if (from) obj.form = from;
        to = requestParam(req, "to");
        if (to) obj.to = to;
        data = requestParam(req, "data");
        if (data) obj.data = data;

        sendData(res, respData, await eth.estimateGas(obj), format);
        break;

      case tokensupply:
        contractaddress = requestParam(req, "contractaddress");
        Contract.findOne({ address: contractaddress }, "totalSupply decimals").exec(
          function (err, doc) {
            if (err) {
              responseFail(res, respData, err.toString());
            } else if (doc == null) {
              ...
              sendData(res, respData, doc.totalSupply / 10**doc.decimals, format);
            }
          }
        );
        break;
      ...
      case masterNodeVer:
        var blockFind = Block.find({ timestamp: { $gt: 1535731200 } }).lean(
          true
        );
        blockFind.exec(function (err, docs) {
          if (err) {
            ...
            for (var j = 0; j < docs.length; j++) {
              var block = docs[j];
              if (!block.witness) continue;
              total++;
              block = filterBlocks(block);
              var ind = keys.indexOf(block.witness);
              var ver =
                block.extraData.charCodeAt(3) +
                "." +
                block.extraData.charCodeAt(4) +
                "." +
                block.extraData.charCodeAt(5);
              if (ind == -1) {
            ...
            var respTable =
              "<table><tr><td>version</td><td>block num</td><td>percent</td></tr>";
            ...
            respTable +=
              "<tr><td> </td><td></td><td></td></tr> <tr><td>witness</td><td>version</td><td></td></tr>";
...
function sendData(res, respData, result, format) {
  let responseObj;
  if (format === 'json') {
