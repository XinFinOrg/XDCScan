...
module.exports = async function (req, res) {
  ...
  try {
    ...
    if (action == "internalTX") {
      ...
      var findCond = {
        $and: [{ to: address }, { from: address }],
        input: { $ne: "0x" },
      };
      Transaction.count(findCond).exec(function (err, c) {
        ...
        transactionFind = Transaction.find(findCond)
          .skip(page * pageSize)
          .limit(pageSize)
          .lean(true);
      ...
      for (let itm of tokenTransfrs) {
        if (itm.resultingTransferArray && itm.resultingTransferArray.value) {
          let temp = etherUnits.toEther(
            new BigNumber(itm.resultingTransferArray.value),
            "wei"
          );
      ...
      if (txHash && txHash.indexOf("0x") != 0) txHash = "0x" + txHash;
      ...
      TransactionFind.exec(function (err, doc) {
        ...
        if (!doc) {
          //if no result in db , get from web3
          var txData = web3relay.getTX(txHash);
          if (txData == null) {
            res.write("{}");
            res.end();
        ...
        if (doc.input && doc.input.length > 2) {
          //contract token
          isContract = true;
          if (
            doc.input.length >= 138 &&
            doc.input.substr(0, 34) == transferMethodFlag
          ) {
            //is transfer method
          ...
          ContractFind.exec(function (contractErr, result) {
            if (contractErr) {
              ...
              if (!doc.to) {
                //contract token creation
                if (isToken) contractLable = "Token Creation";
                else contractLable = "Contract Creation";
              } else {
                if (isToken) {
                  //token transaction
                  contractLable = "Token Transaction";
                } else {
                  //normal contract transaction
